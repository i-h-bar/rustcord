...
impl<'a> Handler {
    pub async fn card_response(
        ...
        match card {
            ...
            Some((card, (front_image, back_image))) => {
                self.send_embed(card, front_image, back_image, &msg, &ctx).await;
            }
        }
    }
    ...
    async fn send_embed(&self, card: FuzzyFound, front_image: Option<Vec<u8>>, back_image: Option<Vec<u8>>, msg: &Message, ctx: &Context) {
        let message = if let Some(front_image) = front_image {
            CreateMessage::new()
                .add_file(
                    CreateAttachment::bytes(
                        front_image, format!("{}.png", card.front_image_id)
                    )
                )
        } else {
            CreateMessage::new()
